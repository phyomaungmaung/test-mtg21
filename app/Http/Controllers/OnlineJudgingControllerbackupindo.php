    ...
    public function saveScore(Request $request,$id)
    {
        ...
        if(!$app){
            ...
            if(strcasecmp($cate_name,"Public Sector")==0){
                ...
                $strategy=(($data['in']/10)*30)+(($data['ps']/10)*30)+
                    (($data['ms']/10)*20)+(($data['cu']/10)*20);
                $sstrategy=($strategy/100)*40;
                ...
                $strategy=(($data['in']/10)*20)+(($data['ps']/10)*30)+
                            (($data['pv']/10)*30)+(($data['cu']/10)*20);
                $sstrategy=($strategy/100)*40;
            ...
            if($form ){
                ...
                if($sav){
                    $msg=lang('Application Form has been save successfully. It is moved to tab Already Judged. We will consider this judgement later.'); 
                    return redirect(route('application.aseanapp'))->with(['success'=>$msg]);
                }
                ...
                if($sav){
                    ...
                    $msg=lang('Application Form has been updated successfully. It is moved to tab Already Judged. We will consider this judgement later.');
                        flash()->success(lang('Application Form has been updated successfully. It is moved to tab Already Judged. We will consider this judgement later.'));
                    return redirect(route('application.aseanapp'))->with(['success'=>$msg]);
                }else{
